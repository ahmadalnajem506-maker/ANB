...
- BULK RECEIPT UPLOAD (background processing via Cloudflare Queues): a client or admin can upload up to 30 receipt photos at once via "Upload Multiple Receipts" on the Expenses screen, instead of one at a time — most useful right after the monthly document reminder (see EMAIL AUTOMATION below), when a client has a whole month's receipts to submit together. Each upload becomes a tracked job (queued → ready → reviewed, or failed) processed in the background, so the uploader doesn't wait — they can close the app immediately after selecting files. Coming back later, a "Uploaded Receipts" list on the Expenses screen shows each job's status; tapping "Review" on a ready job opens the normal expense form pre-filled from the OCR result (same accuracy as live scanning), with the receipt shown full-width directly above the form fields for easy reading without needing to zoom. This complements, rather than replaces, the instant single-photo/single-file scan flow (which still processes synchronously since the user needs the result immediately to review it).
...
- AGREED OVERAGE FEE (optional, always manual, never automatic): a contract can optionally have an agreed overage term pre-configured (a percentage of the base plan, or a fixed € amount), which pre-fills a suggested amount when applying a charge later — but nothing is ever charged automatically, and pre-configuring a term is NOT required to use this feature. There is no automatic usage-based billing of any kind on this app — no counting of invoices/expenses/bank transactions against any threshold, no tolerance percentage, nothing fires on its own. The ⚠️ "Apply Overage Charge" button is available on every active contract's card regardless of whether overage terms were pre-agreed at signing — it's meant for genuinely exceptional, one-off situations, anticipated or not. When a specific client's real usage becomes excessive in a way ANB judges worth billing extra for, the admin discusses it with the client directly first, then deliberately applies a one-time charge for that specific month (amount is pre-filled from the pre-agreed term if one exists, otherwise entered manually) — this creates a record added automatically to the client's next monthly subscription invoice as its own line item (marked so it's never billed twice), and the client is automatically emailed a notification of the charge and reason. If no overage is ever manually applied, nothing beyond the flat monthly price is ever charged, no matter how much the client actually uses the app.
...
- Contracts are open-ended by default (no fixed end date) — they only stop via explicit termination (1-month statutory notice), not through any "renewal" step. New clients go through a signing workflow (agreement sent → client reviews & can request changes or sign → admin approves) before their accounting tabs unlock — while pending, the client only sees a waiting screen plus the ability to review/sign the agreement. Signing can happen two ways: logged in from the client's own dashboard, or via a one-time emailed "magic link" (no login required at all) — sent automatically the moment a new client and their first contract are created, and resendable anytime by the admin via a "📧 Send Signing Link" button on the pending agreement (each send invalidates any previous link and issues a fresh one valid for 7 days).
...
- The same one-time-password mechanism is still available for admins to use whenever they want to hand a client a fresh temporary password directly (Settings → Clients tab, or the client's own screen → "Reset Password").
- ⚠️ SECURITY-BY-DESIGN, DO NOT SUGGEST CHANGING THIS: self-service "Forgot password?" (a one-time emailed reset link, valid 1 hour, single-use, auto-signs the client in once a new password is set) is available on the login screen — but ONLY for CLIENT accounts. Admin accounts deliberately do NOT have self-service reset: the login screen's "Forgot password?" link for an admin shows a message directing them to have another administrator reset it (Settings → Admins → Reset), or contact ANB technical support. This asymmetry is intentional — admin accounts can see every client's financial data, so a compromised admin inbox alone must never be enough to take over an admin account, while a client account only exposes that one client's own data, where the convenience of self-service reset outweighs the smaller risk.
- Any account that logs in with a temporary (admin-issued) password is immediately shown a mandatory, non-dismissible "Set Your Password" screen before anything else in the app becomes usable — there is no way to skip, close, or work around this screen; the account cannot proceed until a new password (min. 6 characters, confirmed twice) is successfully saved.
- Separately, a client can voluntarily change their own password any time from their dashboard's Security card ("Change Password") — this requires entering their CURRENT password correctly first (server-verified) before the new password (min. 6 characters, confirmed twice) is accepted. Changing a password this way (self-service change, self-service reset via email, or an admin-initiated reset) immediately invalidates any other active login sessions for that account on other devices — only the device that just set the new password stays signed in.
...
EMAIL AUTOMATION (via Resend, on the anbfinancial.nl domain — added as part of "ANB AutoStack"):
- PREFERRED LANGUAGE: every client has a "Preferred Language" field (Nederlands / English / العربية, set in Add Client or Edit Client, defaults to Dutch) that drives the language of every automated email sent to them below. This is separate from the in-app UI language toggle (which is a personal per-device preference) and separate from the contract document itself — the signed service agreement text is always in Dutch regardless of this setting, since that's standard for Dutch service contracts; only the email wrapper around it changes language, with an explicit note in every language stating the contract itself is in Dutch.
- WELCOME EMAIL: the moment a brand-new client and their first contract are created, they're automatically emailed a signing link for their service agreement (see the magic-link signing flow under CONTRACTS & PRICING above) — this doubles as their welcome email, so no separate "Add Contract" step is needed just to notify them.
- MONTHLY DOCUMENT REMINDER: on the 1st of every month, every active client is automatically emailed (in their preferred language) a reminder to email last month's bank statement export to info@anbfinancial.nl and to upload last month's expense receipt photos directly in the app under Expenses.
- CLIENT NOTIFICATIONS: a client is automatically emailed when (a) their agreement/contract is approved by the admin and becomes active (states the new monthly price), and (b) an overage charge is applied to their contract (states the amount, month, and reason if one was noted).
- These are all one-way transactional emails sent from info@anbfinancial.nl; a failed send never blocks or reverses the underlying action (client creation, contract approval, overage charge) — it only means that one notification didn't go out, and the admin can always follow up manually if needed.
